...
/// @author scx567888
/// @version 0.0.1
final class ScxEventWebSocketImpl implements ScxEventWebSocket {

    private static final Logger LOGGER = getLogger(ScxEventWebSocketImpl.class.getName());

    private final ScxWebSocket messageWebSocket;
    ...
    private Consumer<WebSocketCloseInfo> closeHandler;
    private Consumer<Throwable> errorHandler;
    private volatile boolean running;

    public ScxEventWebSocketImpl(ScxWebSocket messageWebSocket) {
        this(messageWebSocket, null);
    }

    public ScxEventWebSocketImpl(ScxWebSocket messageWebSocket, Executor callbackExecutor) {
    ...
    @Override
    public ScxEventWebSocket onClose(Consumer<WebSocketCloseInfo> closeHandler) {
    ...
    public void start() {
        ...
        while (running) {
            try {
                // 尝试读取 帧
                var message = messageWebSocket.read();
                // 处理帧
    ...
    @Override
    public void send(WebSocketMessage message) throws WebSocketIOException, WebSocketInvalidStateException, WebSocketProtocolException {
        this.messageWebSocket.send(message);
    }
    ...
    private void _handleException(Exception exception) {
        ...
        WebSocketCloseInfo localCloseInfo;
        WebSocketCloseInfo peerCloseInfo;
        if (exception instanceof WebSocketProtocolException we) {
            // 此处假设 we.getMessage() 永远小于 125 字节.
            localCloseInfo = new WebSocketCloseInfo(we.closeCode(), we.getMessage());
            peerCloseInfo = new WebSocketCloseInfo(we.closeCode(), we.getMessage());
        } else {
    ...
    private void _handleMessage(WebSocketMessage message) {
        switch (message) {
            case TextMessage textMessage -> _handleText(textMessage);
            case BinaryMessage binaryMessage -> _handleBinary(binaryMessage);
            case PingMessage pingMessage -> _handlePing(pingMessage);
            case PongMessage pongMessage -> _handlePong(pongMessage);
            case CloseMessage closeMessage -> _handleClose(closeMessage);
        }
    }

    //******************* _handleXXX **********************

    private void _handleText(TextMessage textMessage) {
        try {
            _callOnText(textMessage.text());
        } catch (Exception e) {
            LOGGER.log(ERROR, "Error while calling onTextMessage : ", e);
        }
    }

    private void _handleBinary(BinaryMessage binaryMessage) {
        try {
            _callOnBinary(binaryMessage.binary());
        } catch (Exception e) {
            LOGGER.log(ERROR, "Error while call onBinaryMessage : ", e);
        }
    }

    private void _handlePing(PingMessage pingMessage) {
        try {
            _callOnPing(pingMessage.data());
        } catch (Exception e) {
            LOGGER.log(ERROR, "Error while call onPing : ", e);
        }
    }

    private void _handlePong(PongMessage pongMessage) {
        try {
            _callOnPong(pongMessage.data());
        } catch (Exception e) {
            LOGGER.log(ERROR, "Error while call onPong : ", e);
        }
    }

    private void _handleClose(CloseMessage closeMessage) {
        try {
            _callOnClose(closeMessage.closeInfo());
        } catch (Exception e) {
            LOGGER.log(ERROR, "Error while call onClose : ", e);
        }
    ...
    private void _callOnClose(WebSocketCloseInfo closeInfo) {
